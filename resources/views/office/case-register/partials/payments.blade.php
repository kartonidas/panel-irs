<div class="p-3 border-start border-bottom border-end">
    <div class="text-end mb-2">
        <button type="button" class="btn btn-primary btn-sm open-modal" data-modal="#paymentModal" data-default-values='{!! json_encode(["currency" => "PLN"]) !!}'>{{ __("Dodaj wpłatę") }}</button>
    </div>
            ...
            <table class="table table-stripped table-hover" id="case-schedule">
                <thead>
                    <tr>
                        ...
                        <th style="width: 220px;" class="text-end sortable" data-field="amount_pln">{{ __("Kwota") }}</th>
            ...
            <form method="post" action="{{ route("office.case_register.payment.post", $case->id) }}" onsubmit="Case.savePaymentForm(this); return false;">
                <div class="modal-body py-0">
                    <div class="row g-3">
                        ...
                        <div class="col-4">
                            <label for="formPaymentAmount" class="form-label">{{ __("Waluta") }}*</label>
                            <select id="formPaymentAmount" class="form-select" name="currency" data-validate="required">
