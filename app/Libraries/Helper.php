...
class Helper
{
    ...
    public static function setFilter(Request $request, $module)
    {
        $filterFields = self::getFilterFields($module);
        ...
        if($filterFields)
        {
            ...
            $request->session()->put("Filter:" . $module, serialize($filterParams));
        }
    }

    public static function clearFilter(Request $request, $module)
    {
        $request->session()->forget("Filter:" . $module);
    }

    public static function getFilter(Request $request, $module, $fromSession = true)
    {
        $out = [];
        $filterFields = self::getFilterFields($module);
        if($filterFields)
        {
            if($fromSession)
            {
                $filter = $request->session()->get("Filter:" . $module, "");
                $filter = $filter ? unserialize($filter) : [];
            }
            else
                $filter = $request->all();

            $defaultValues = self::getDefaultFilterValues($module);
    ...
    public static function getFilterFields($module)
    {
        return config("filter.fields")[$module] ?? [];
    }

    private static function getDefaultFilterValues($module)
    {
        return config("filter.default")[$module] ?? [];
    }

    public static function setSortOrder(Request $request, $module, $sort)
    {
        $sortArray = [];
        $sortFields = self::getSortFields($module);
        if($sortFields)
        {
            $sort = explode(",", $sort, 2);
            $sort = array_map("strtolower", $sort);

            $sortArray[0] = !empty($sort[0]) && in_array($sort[0], $sortFields["fields"]) ? $sort[0] : $sortFields["default"][0];
            $sortArray[1] = !empty($sort[1]) && in_array($sort[1], ["asc", "desc"]) ? $sort[1] : $sortFields["default"][1];

            $request->session()->put("Sort:" . $module, serialize($sortArray));
        }
    }

    public static function getSortOrder(Request $request, $module, $default = [], $fromSession = true)
    {
        $out = $default;
        $sortFields = self::getSortFields($module);
        if($sortFields)
        {
            if($fromSession)
            {
                $sort = $request->session()->get("Sort:" . $module, "");
            ...
            $sortFields = self::getSortFields($module);
            $out = $sortFields["default"];
            if(!empty($sort[0]) && in_array($sort[0], $sortFields["fields"]))
                $out[0] = $sort[0];
    ...
    public static function getSortFields($module)
    {
        $out = [];
        switch($module)
        {
            case "backend:users":
                $out["fields"] = \App\Models\User::$sortable;
                $out["default"] = \App\Models\User::$defaultSortable;
            break;
        
            case "backend:debts":
                $out["fields"] = \App\Models\Debt::$sortable;
                $out["default"] = \App\Models\Debt::$defaultSortable;
            break;
        
            case "backend:invoices":
                $out["fields"] = \App\Models\Invoice::$sortable;
                $out["default"] = \App\Models\Invoice::$defaultSortable;
            break;
    ...
    public static function getAllSortFields(Request $request, $module, $sort)
    {
        $out = [];
        $fields = self::getSortFields($module);
        if($fields)
        {
    ...
    public static function setPageSize(Request $request, $module, $size)
    {
        $default = config("site.lists.size");
        $allowed = config("site.lists.sizes");
        if(preg_match("/^backend:/i", $module))
        {
            $default = config("backend.lists.size");
            $allowed = config("backend.lists.sizes");
        }
        
        if(!in_array($size, $allowed))
            $size = $default;

        $request->session()->put("PageSize:" . $module, $size);
    }

    public static function getPageSize($module)
    {
        $default = config("site.lists.size");
        if(preg_match("/^backend:/i", $module))
            $default = config("backend.lists.size");
            
        return request()->session()->get("PageSize:" . $module, $default);
    }
